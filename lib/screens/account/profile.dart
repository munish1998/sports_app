...
class _ProfileScreenState extends State<ProfileScreen>
                          ...
                          child: Center(
                            child: CardSwiper(
                                ...
                                cardBuilder: (context, index, o, i) {
                                  var item = PersonalizeModel(
                                      id: userIdKey,
                                      name: data.profileModel!.name,
                                      title: 'welcome',
                                      rewards: '500',
                                      rank: '0',
                                      image: data.profileModel!.profilePicture,
                                      completionDate: '15-09-2023');
                                  log('response of item=====>>>>$item');
                                  return InkWell(
                                    onTap: () {
                                      showCardDetail(
  ...
  _peeronalizecard(BuildContext context, ProfileProvider item) {
    return Column(
      children: [
        Center(
          child: Container(
            ...
            child: Center(
              child: CardSwiper(
                numberOfCardsDisplayed:
                    (item.personalizeCards.length > 1) ? 2 : 1,
                cardsCount: item.personalizeCards.length,
                cardBuilder: (context, index, o, i) {
                  var items = item.personalizeCards[index];
                  return cardWidget(items);
                },
              ),
            ),
          ),
  ...
  Column _personalizecard(BuildContext context, ProfileProvider data) {
    return Column(
      children: [
        Center(
          child: Container(
            ...
            child: Center(
              child: CardSwiper(
                  ...
                  cardBuilder: (context, index, o, i) {
                    ...
                    return InkWell(
                      onTap: () {
                        showCardDetail(context: context, item: item);
                      },
                      child: cardWidget(item),
                    );
                  }),
  ...
  Future<void> _imageCropper(File photo) async {
    ...
    setState(() {
      if (cropPhoto != null) {
        ...
        log('selectedImage===================>>>${_selectedImage!.path}');
        log('selectedImagepath=======================>>>${_selectedImage!.path.toString().split('/').last.replaceAll('\'', '')}');
      }
    });
  }
