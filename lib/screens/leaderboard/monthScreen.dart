...
class _MonthScreenState extends State<MonthScreen> {
  ...
  Widget get tabviewMonth => SingleChildScrollView(
        ...
        child: Consumer<LeaderboardProvider>(builder: (context, data, child) {
          return Column(
            children: [
              ...
              levelSection,
              topLeaders(data.leaderMonthTop),
              if (data.leaderMonthTop.isNotEmpty)
                AppImage("assets/linedivider.svg"),
              if (data.leaderMonthTop.isNotEmpty) locationSection,
              if (data.leaderMonthTop.isNotEmpty)
                AppImage("assets/linedivider.svg"),
              SizedBox(
                height: 20,
              ),
              listLeaders(data.leaderboardMonthList),
            ],
          );
        }),
      );

  Widget get locationSection => Container(
  ...
  void onSelectLocation(String type, String location) async {
    ...
    pro.getLeaderboardMonth(context: context, data: data, isHome: false);
  }
